...
const OPTEL_SLOT_MONTH_COLS = [
  "P2 DES", "JAN", "FEB", "MAR", "APR", "MEI", "P1 JUN",
] as const;
...
interface OptelColumnMap {
  npk: number;
  nama: number;
  fungsi: number;
  statusKemitraan: number;
  slotMonths: number[];        // accumulation slot months
  totalSlot: number;
  regularSlotMonths: number[];
  totalSlotReguler: number;
  downgrade: number;
  reset: number;
  token: number;
  grading: number;
  jenisMembership: number;
  statusResign: number;
}
...
export async function parseOptelXLSX(buffer: Uint8Array | Buffer): Promise<ParseResult<ParsedOptelRow>> {
  ...
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment`n  // @ts-expect-error ExcelJS Buffer type mismatch with newer @types/node`n  await workbook.xlsx.load(buffer as never);
