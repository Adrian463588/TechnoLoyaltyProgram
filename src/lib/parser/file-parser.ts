...
export class FileParser {
   ...
   */
  static parseCsv(csvBuffer: Buffer | string): Record<string, any>[] {
    ...
    return parsed.data.map((row: any) => normalizeRowKeys(row));
  }

  /**
   * Parses an XLSX Buffer into an array of normalized objects.
   */
  static parseXlsx(xlsxBuffer: Buffer): Record<string, any>[] {
    ...
    return rawData.map((row: any) => normalizeRowKeys(row));
  }
  
  /**
   * Universal parse method that handles routing to the correct parser based on filename.
   */
  static parseBuffer(buffer: Buffer, filename: string): Record<string, any>[] {
