...
describe('AdminDashboardPage', () => {
  it('renders the dashboard with correct layout', () => {
    ...
    expect(screen.getByText('HC PM Control')).toBeInTheDocument()
    
    // Check for operational metrics
    expect(screen.getByText('Active Period')).toBeInTheDocument()
    expect(screen.getByText('Last Cut-Off')).toBeInTheDocument()
    expect(screen.getAllByText('Pending')[0]).toBeInTheDocument()
    
    // Check for upload module
    expect(screen.getByText('Data Injection')).toBeInTheDocument()
